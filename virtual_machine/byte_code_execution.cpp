...
void virtual_machine::execute_BYTE_CODE_COPY_LAST_PREVIOUS_STACK_DATA(int __arg, Execution* __execution) {
    ...
    if (!__arg) __execution->stacks->last->object->stack->printContent();

    delete __execution->stacks->last->previous->object->stack->remove(__execution->stacks->last->previous->object->stack->count);
